 ...
 * -----------------------------------------------------------------------
 *   /com/wyvernzora/centipede/states/GameOverState.java
 * -----------------------------------------------------------------------
 * 
 * The screen you see after game over
 * 
...
public class GameOverState extends GameState {
	...
	public GameOverState(Game gm) {
		super(gm);

		/*this.addComponent(new DrawableGameComponent(game){

			Rectangle bounds = Utilities.reScale(
					new Rectangle(136, 150, 328, 44)
					, 20, CentipedeGame.TILE_SIZE);
			Image img = ContentPipeline.getResourceNoBuffer("GAME_OVER");	
			...
			public void draw(Graphics2D g) {
				g.drawImage(img, bounds.x, bounds.y, bounds.x + bounds.width, bounds.y + bounds.height,
						0, 0, 328, 44, game.getImageObserver());
				}
		...
		this.addComponent(new DrawableGameComponent(game){
			...
			@Override
			public void draw(Graphics2D g) {
				TextGraphics.drawOutlinesStringCentered(g, "Game Over", fnt, Color.WHITE, Color.RED, 3.0f, center);
			}
		...
		TextButton backToTitle = new TextButton(game, Utilities.reScale(
				...
				CentipedeGame.TILE_SIZE), rank >= 0 ? "Continue" : "Back To Title",
			...
			protected void onMouseClick(MouseState oldState, MouseState newState) {
				if (oldState.isButtonDown(MouseEvent.BUTTON1)
					...
					if (rank >= 0){
						...
						HighScore.instance().insertHighScore(entry, rank);
